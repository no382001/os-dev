...
/*
 * given a pci device(32-bit vars containing info about bus, device number, and
 * function number), a field(what u want to read from the config space) Read it
 * for me !
 * */
uint32_t pci_read(pci_dev_t dev, uint32_t field) {
  ...
  if (size == 1) {
    // get the first byte only, since it's in little endian, it's actually the
    // 3rd byte
...
/*
 * The following three functions are basically doing recursion, enumerating each
 *and every device connected to pci We start with the primary bus 0, which has 8
 *function, each of the function is actually a bus Then, each bus can have 8
 *devices connected to it, each device can have 8 functions When we gets to
 *enumerate the function, check if the vendor id and device id match, if it
 *does, we've found our device !
 **/
...
pci_dev_t pci_scan_function(uint16_t vendor_id, uint16_t device_id,
  ...
  // If it's a PCI Bridge device, get the bus it's connected to and keep
  // searching
 ...
 * Scan device
 * */
pci_dev_t pci_scan_device(uint16_t vendor_id, uint16_t device_id, uint32_t bus,
                          uint32_t device, int device_type) {
  pci_dev_t dev = {0};
  dev.bus_num = bus;
  dev.device_num = device;
  ...
  pci_dev_t t =
      pci_scan_function(vendor_id, device_id, bus, device, 0, device_type);
  if (t.bits)
    return t;
 ...
 * Scan bus
 * */
pci_dev_t pci_scan_bus(uint16_t vendor_id, uint16_t device_id, uint32_t bus,
                       int device_type) {
  for (int device = 0; device < DEVICE_PER_BUS; device++) {
    pci_dev_t t =
        pci_scan_device(vendor_id, device_id, bus, device, device_type);
    if (t.bits)
      return t;
  }
  return dev_zero;
...
/*
 * Device driver use this function to get its device object(given unique vendor
 * id and device id)
 * */
